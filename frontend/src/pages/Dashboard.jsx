...
import { Video, Star, DollarSign, BookOpen, LogOut, Plus } from "lucide-react";
import SessionCard from "./SessionCard";
import TutorCard from "./TutorCard"; // your other component
import useWebsocket, { getSocket } from "../hooks/useWebsocket";
...
const Dashboard = ({ user, onJoinCall, onSchedule, onProfileEdit, onLogout }) => {
  ...
  const handleWsMessage = (data) => {
    if (!data) return;
    if (data.type === "session-started" || data.type === "session-started-global") {
      const { sessionId } = data;
      setSessions((prev) =>
        prev.map((s) => (s._id === sessionId ? { ...s, status: "live" } : s))
      );
    }
    // other event handling (user-joined, user-left) can be added
  };

  // attach WS using the hook
  ...
  const enrollInSession = async (sessionId) => {
    try {
      ...
      const res = await axios.post(`${backendUrl}/api/sessions/${sessionId}/enroll`, {}, config);

      setSessions((prev) => prev.map((item) => (item._id === sessionId ? res.data : item)));
      alert("Successfully enrolled in session 🎉");
    } catch (error) {
      console.error("Error enrolling in session", error);
  ...
  const startLiveSession = async (sessionId) => {
    try {
      ...
      const res = await axios.patch(`${backendUrl}/api/sessions/${sessionId}/start`, {}, config);

      // 2) Update local UI
      setSessions((prev) => prev.map((item) => (item._id === sessionId ? res.data : item)));

      // 3) Notify via WebSocket: server will broadcast to learners
      const ws = getSocket();
      if (ws && ws.readyState === WebSocket.OPEN) {
        ws.send(JSON.stringify({ type: "start-session", sessionId, actorId: user._id }));
      } else {
        // If socket not open yet, try to open then send
        ws.addEventListener("open", () => {
          ws.send(JSON.stringify({ type: "start-session", sessionId, actorId: user._id }));
        });
      }

      // Optionally take the tutor into the live UI
      onJoinCall(res.data);
    } catch (error) {
      console.error("Error starting live session", error);
    }
        ...
        {tabIndex === 1 && (
          <Box>
            <Box sx={{ display: "flex", flexDirection: { xs: "column", sm: "row" }, justifyContent: "space-between", mb: 2 }}>
              ...
              {user.role === "tutor" && (
                <Button variant="contained" startIcon={<Plus />} onClick={onSchedule} fullWidth={isMobile}>
                  New Session
                </Button>
              )}
            </Box>
