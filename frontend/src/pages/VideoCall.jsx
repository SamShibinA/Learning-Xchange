...
const VideoCall = ({ session, user, onLeave, backendUrl }) => {
  ...
  // ✅ Decide grid layout based on number of participants
  const videoCount = remoteStreams.length + 1; // local + remotes
  const getGridTemplate = () => {
    if (videoCount <= 1) return "1fr";
    if (videoCount === 2) return "1fr 1fr";
    if (videoCount <= 4) return "1fr 1fr";
    if (videoCount <= 6) return "1fr 1fr 1fr";
    if (videoCount <= 9) return "1fr 1fr 1fr";
